function [T,E_v,E_iNaBM_m,E_iNaBM_h,E_iKBM_n,E_E_iAMPACOBAHHtanh_sAMPA,E_E_iAMPACOBAHHtanh_netcon]=solve_ode
% ------------------------------------------------------------
% Parameters:
% ------------------------------------------------------------
...
for k=2:ntime
  t=T(k-1);
  E_v_k1=p.E_Iapp+((-(( p.E_iNaBM_gNa.*E_iNaBM_m(n-1).^3.*E_iNaBM_h(n-1).*(E_v(n-1)-p.E_iNaBM_ENa))))+((-(( p.E_iKBM_gK.*E_iKBM_n(n-1).^4.*(E_v(n-1)-p.E_iKBM_EK))))+((-(( p.E_iLeakBM_gLeak*(E_v(n-1)-p.E_iLeakBM_ELeak))))+((-(( p.E_E_iAMPACOBAHHtanh_gAMPA/p.E_Npop.*(E_E_iAMPACOBAHHtanh_sAMPA(n-1)*E_E_iAMPACOBAHHtanh_netcon).*((E_v(n-1))-p.E_E_iAMPACOBAHHtanh_EAMPA))))))));
