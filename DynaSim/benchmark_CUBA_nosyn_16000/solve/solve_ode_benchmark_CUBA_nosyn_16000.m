...
for k=2:ntime
  ...
  I_v_k1=-p.I_gLeak*(I_v(n-1,:)-p.I_ELeak)./p.I_taum;
  % ------------------------------------------------------------
  ...
  conditional_test=(I_v(n,:)>-50);
  if any(conditional_test), I_v(n,conditional_test)=-60; end
  n=n+1;
end
T=T(1:downsample_factor:ntime);
fprintf('Sim Time: %g seconds\n', toc);
end
